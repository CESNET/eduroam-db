...
function init_functions($scope, $http, $timeout)
{
  ...
  //  console.log("collapsing some accordion");
  //})

  //$scope.add_collapse_event = function() {
 ...
 $(document).on('click','.collapse',function(){ 
    console.log("click event on .collapse");
 });

 $(document).on('collapse','.collapse',function(){ 
    console.log("collapse event on .collapse");
 });
...
function save_json_to_api($scope, $http)
{
  ...
  .then(function(response) {
    ...
    if(err.status == 400) {
      $scope.error = "Nastala chyba při validaci ukládaných dat.";
